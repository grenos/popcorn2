...
export const getUserInputMovies = async (value: string, page: number): Promise<{}> => {
  const res = await axios.get(`/search/movie?api_key=${apikey}&language=en-US&sort_by=popularity.desc&include_adult=false&query=${value}&page=${page}`)
  return res
}
export const getUserInputSeries = async (value: string, page: number): Promise<{}> => {
  const res = await axios.get(`/search/tv?api_key=${apikey}&language=en-US&sort_by=popularity.desc&include_adult=false&query=${value}&page=${page}`)
  return res
}
